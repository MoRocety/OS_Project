...
@app.route('/simulate')
def simulate():
    simulation_output = []  # Capture the print statements here
    ...
    while (yet_to_arrive_queue or ready_queue) or (current_process is not None):
        ...
        while yet_to_arrive_queue and yet_to_arrive_queue[0].arrival_time <= current_time:
            ...
            if not current_process:
                ...
                simulation_output.append(f"At Time {current_time} => Process {process.process_id} added to idle CPU.")
            ...
            # Preemption, Process added back to ready queue
                ...
                simulation_output.append(f"At Time {current_time} => Context switched, Process {current_process.process_id} replaced old Process {old_process.process_id}.")
                
            # No preemption, Process added to ready queue
            else:
                simulation_output.append(f"At Time {current_time} => Process {process.process_id} added to ready queue.")
        ...
        if current_process:
            ...
            if current_process.golden_runtime == 0:
                simulation_output.append(f"At Time {current_time} => Process {current_process.process_id} has finished executing.")
    ...
    return jsonify(simulation_output)
